...
class BoSelectaTagLib extends ConfService implements ClientSessions {
	...
	def connect  =  { attrs ->
		...
		loadTemplate(attrs,'socketConnect', cBean,null)
		if (cBean.autodisco) {
			clientListenerService.disconnect(oSession)
		}
		loadTemplate(attrs,'socketProcess', cBean,null)
	}
	...
	def selecta = {attrs ->
		...
		//AutoComplete
		if (bo.autoComplete) {
			loadTemplate(attrs,'genAutoComplete', bo,null)
			//Select
		}else{
		...
		if (bo.value||bo.nextValue) {
			...
			loadTemplate(attrs,'actionNonAppendThis', null,map)
		}
		if (bo.loadPrimary && (bo.formatting == "JSON")) {
			Map jsonMap = [primarylist: bo.primarylist, id:id, formatting: bo.formatting, updateValue:bo.value]
			loadTemplate(attrs,'primaryJSONParser', null, [message: jsonMap as JSON])
		}
	}
	...
	def selecta2 = {attrs ->
		...
		if (autoComplete) {
			...
			loadTemplate(attrs,'genAutoComplete', null,map)
			//select function
		}else{
	...
	private void loadTemplate(attrs, String template, def cBean, Map myMap) {
